...
export default function PrinterPage() {
  ...
  const [formData, setFormData] = useState({
    name: '',
    printer_type: 'ip', // 'ip' or 'usb'
    ip_address: '',
    port: '9100',
    usb_port: 'COM3', // Default USB port for Windows
    is_default: false
  })
...
const handleSubmit = async (e) => {
  ...
  const handleEdit = (printer) => {
    setFormData({
      name: printer.name,
      printer_type: printer.printer_type || 'ip',
                        ...
                        <div className="flex items-center space-x-2">
                          <motion.button
                            ...
                            onClick={() => testPrinterConnection(
                              ...
                              printer.printer_type === 'usb' ? printer.usb_port : printer.ip_address,
                              printer.port,
                              true
                            )}
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  <Network className={`w-6 h-6 mx-auto mb-2 ${formData.printer_type === 'ip' ? 'text-purple-600' : classes.textSecondary}`} />
                  <div className={`text-sm font-medium ${formData.printer_type === 'ip' ? 'text-purple-600' : classes.textPrimary}`}>
                    Network (IP)
                  </div>
                  <div className={`text-xs ${classes.textSecondary} mt-1`}>Ethernet/WiFi printer</div>
                </motion.button>
                ...
                >
                  <Usb className={`w-6 h-6 mx-auto mb-2 ${formData.printer_type === 'usb' ? 'text-purple-600' : classes.textSecondary}`} />
                  <div className={`text-sm font-medium ${formData.printer_type === 'usb' ? 'text-purple-600' : classes.textPrimary}`}>
                    USB/Serial
                  </div>
                  <div className={`text-xs ${classes.textSecondary} mt-1`}>Direct USB connection</div>
                </motion.button>
              </div>
            </div>

...
useEffect(() => {
  ...
  // CRITICAL: Set user ID in printerManager and customerLedgerManager immediately
  if (userData?.id) {
    printerManager.setUserId(userData.id)
    customerLedgerManager.setUserId(userData.id)
    console.log('✅ [Payment Page] User ID set in printerManager and customerLedgerManager:', userData.id)
  }
      ...
      if (selectedPaymentMethod?.id === 'account' && orderData.customer?.id) {
        try {
          ...
          if (ledgerError) {
            ...
            console.log(`✅ [Payment] Updated ledger entry created: Rs ${orderData.total} (Balance: ${currentBalance} -> ${newBalance})`)
...
const handlePrintKitchenToken = async () => {
  ...
  const handleNewOrder = () => {
    // Always navigate to dashboard when New Order is clicked
    router.push('/dashboard')
  }
          ...
          <div className="flex flex-col gap-3 overflow-y-auto">
            ...
            <div className={`${classes.card} rounded-lg ${classes.border} border p-2 flex-1 overflow-hidden flex flex-col`}>
              ...
              {selectedPaymentMethod?.id === 'account' && orderData.customer && (
                <div className={`mb-2 p-2 rounded-lg border ${isDark ? 'bg-blue-900/20 border-blue-700/30' : 'bg-blue-50 border-blue-200'}`}>
                  <div className="flex items-center justify-between">
                    <div>
                      <p className={`text-xs font-semibold ${isDark ? 'text-blue-300' : 'text-blue-900'}`}>
                        {orderData.customer.first_name} {orderData.customer.last_name}
                      </p>
                      <p className={`text-[10px] ${isDark ? 'text-blue-400' : 'text-blue-700'}`}>
                        Current Balance: {loadingLedgerBalance ? 'Loading...' : `Rs ${customerLedgerBalance.toFixed(2)}`}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className={`text-xs ${classes.textSecondary}`}>After Order:</p>
                      <p className={`text-sm font-bold ${isDark ? 'text-orange-400' : 'text-orange-600'}`}>
                        Rs {(() => {
                          // CRITICAL FIX: For modified orders, subtract old amount before adding new amount
                          // to avoid double-counting (old balance already includes original order)
                          if (orderData.isModifying && orderData.detailedChanges?.oldTotal) {
                            // Modified order: remove old amount, add new amount
                            const afterOrderBalance = customerLedgerBalance - orderData.detailedChanges.oldTotal + orderData.total
                            return afterOrderBalance.toFixed(2)
                          } else {
                            // New order: just add the total
                            return (customerLedgerBalance + orderData.total).toFixed(2)
                          }
                        })()}
                      </p>
                    </div>

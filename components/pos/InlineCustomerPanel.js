...
export default function InlineCustomerPanel({
  ...
  const handleCreateCustomer = () => {
    ...
    try {
      const existing = JSON.parse(localStorage.getItem('pos_customers') || '[]')
      existing.push(newCustomer)
      localStorage.setItem('pos_customers', JSON.stringify(existing))
    ...
    <div className="mt-1.5">
      ...
      {(suggestions.length > 0 || showCreateOption || showNewCustForm) && (
        <div className={`mt-1 rounded-lg border overflow-hidden ${isDark ? 'bg-gray-800 border-gray-600' : 'bg-white border-gray-200'}`}>
          ...
          {showNewCustForm && (
            <div className={`px-3 py-2 space-y-1.5 border-t ${isDark ? 'border-gray-700 bg-gray-800/80' : 'border-gray-100 bg-purple-50/60'}`}>
              <div className={`text-xs font-semibold mb-1 ${isDark ? 'text-purple-300' : 'text-purple-700'}`}>New Customer</div>
              <input
                ref={nameInputRef}
                value={newCustName}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1 mb-1.5">
              {['Home', 'Office', 'House 1', 'House 2', 'Other'].map(lbl => (
                <button
                  key={lbl}
                  onClick={() => update('addressLabel', lbl)}

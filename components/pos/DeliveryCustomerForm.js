...
export default function DeliveryCustomerForm({
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (
        ...
        !addressSelectorRef.current.contains(event.target)
      ) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Check if this is a new address for existing customer
      const isNewAddress = selectedCustomer?.id &&
        !customerAddresses.some(a => a.address_line === formData.addressLine.trim())

      // Save new address if it's different from existing ones
      if (savedCustomer.id && isNewAddress && formData.addressLine.trim()) {
        await saveNewAddress(
          savedCustomer.id,
          formData.addressLine,
          formData.addressLabel,
          customerAddresses.length === 0 // Make default if first address
        )
      }
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className={`relative w-full max-w-md h-full ${isDark ? 'bg-gray-900' : 'bg-white'} shadow-2xl flex flex-col`}>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {/* Delivery Address */}
          <div className="relative">
            <label className={`block text-sm font-semibold mb-2 flex items-center justify-between ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>
              <span className="flex items-center">
                <MapPin className="w-4 h-4 mr-2 text-red-500" />
                Delivery Address *
              </span>
              {/* Show saved addresses count if customer has multiple */}
              {selectedCustomer && customerAddresses.length > 0 && (
                ...
                >
                  ...
                  {customerAddresses.length} saved address{customerAddresses.length > 1 ? 'es' : ''}
                </button>
              )}
            </label>

            {/* Address Selector Dropdown */}
            {showAddressSelector && customerAddresses.length > 0 && (
              <div
                ref={addressSelectorRef}
                className={`absolute top-8 right-0 left-0 mt-1 rounded-lg shadow-xl overflow-hidden z-50 ${
                  isDark ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'
                }`}
              >
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                          ...
                          <AddrIcon className={`w-4 h-4 ${isDark ? 'text-purple-400' : 'text-purple-600'}`} />
                        </div>
                        <div className="flex-1 min-w-0">
                ...
                <button
                  onClick={() => {
                    ...
                    setFormData(prev => ({ ...prev, addressLine: '', addressLabel: 'Home' }))
                  }}
            ...
            {/* Address Label (for new addresses) */}
            {(showAddNewAddress || !selectedCustomer || customerAddresses.length === 0) && (
              <div className="flex gap-2 mb-2">
                <button
                  ref={homeButtonRef}
                  type="button"
                  onClick={() => setFormData(prev => ({ ...prev, addressLabel: 'Home' }))}
                  onKeyDown={handleKeyDown}
                ...
                <button
                  ref={officeButtonRef}
                  type="button"
                  onClick={() => setFormData(prev => ({ ...prev, addressLabel: 'Office' }))}
                  onKeyDown={handleKeyDown}
                  className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-all ${
                    formData.addressLabel === 'Office'
                      ? isDark
                        ? 'bg-purple-600 text-white'
                        : 'bg-purple-500 text-white'
                      : isDark
                ...
                <button
                  ref={house1ButtonRef}
                  type="button"
                  onClick={() => setFormData(prev => ({ ...prev, addressLabel: 'House 1' }))}
                  onKeyDown={handleKeyDown}
                  className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-all ${
                    formData.addressLabel === 'House 1'
                      ? isDark
                        ? 'bg-purple-600 text-white'
                        : 'bg-purple-500 text-white'
                      : isDark
                ...
                <button
                  ref={house2ButtonRef}
                  type="button"
                  onClick={() => setFormData(prev => ({ ...prev, addressLabel: 'House 2' }))}
                  onKeyDown={handleKeyDown}
                  className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-all ${
                    formData.addressLabel === 'House 2'
                      ? isDark
                        ? 'bg-purple-600 text-white'
                        : 'bg-purple-500 text-white'
                      : isDark
                ...
                <button
                  ref={otherButtonRef}
                  type="button"
                  onClick={() => setFormData(prev => ({ ...prev, addressLabel: 'Other' }))}
                  onKeyDown={handleKeyDown}
                  className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-all ${

...
class WebOrderNotificationManager {
  ...
  async getPendingCount() {
    ...
    try {
      ...
      console.error('Error fetching pending web orders count:', error);
      return 0;
    }
  }
  ...
  startListening(onNewOrder, options = {}) {
    ...
    if (this.channel) {
      console.log('⚠️ [WebOrderNotif] Already listening for web orders');
      return;
    }

    console.log('📡 [WebOrderNotif] Starting to listen for new web orders');
    ...
    this.channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // Check if it's a website order
          if (payload.new?.original_order_source === 'Website' && payload.new?.is_approved === false) {
            console.log('🌐 [WebOrderNotif] ✅ New web order detected:', payload.new.order_number);
            ...
            console.log('ℹ️ [WebOrderNotif] Order event but not a pending web order:', {
  ...
  stopListening() {
    if (this.channel) {
      console.log('📡 [WebOrderNotif] Stopping web order notifications');

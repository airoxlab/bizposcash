  ...
  async login(phone, password) {
    try {
      ...
      if (cashier && !cashierError && cashier.password === password) {
        ...
        // Download printer assets (non-blocking - don't await)
        this.downloadPrinterAssets(cashier.users).catch(err => {
          console.error('❌ Printer assets download failed (non-blocking):', err)
        })
...
async getOrderHistory(orderId) {
  try {
    ...
    if (cachedHistory && cachedHistory.length > 0) {
      console.log(`✅ [AuthManager] Using cached order history (${cachedHistory.length} entries)`)
      return cachedHistory
    }

    // If online, fetch from database
    if (!navigator.onLine) {
      console.log('📴 [AuthManager] Offline and no cached history available')
      return []
    }
    ...
    // Cache the fetched history for offline use
    if (data && data.length > 0) {
      cacheManager.setOrderHistory(orderId, data)
      console.log(`✅ [AuthManager] Cached ${data.length} history entries for order ${orderId}`)
    }

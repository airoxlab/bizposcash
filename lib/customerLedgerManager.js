...
class CustomerLedgerManager {
  ...
  async getCustomerBalance(customerId) {
    // Offline fast-path — return cached balance immediately
    if (typeof navigator !== 'undefined' && !navigator.onLine) {
      const cached = this._getCachedBalance(customerId)
      console.log(`📴 [Ledger] Offline — returning cached balance for customer ${customerId}: Rs ${cached}`)
      return cached
    }

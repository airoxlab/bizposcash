  ...
  async syncPrintersFromDatabase() {
    try {
      ...
      for (const dbPrinter of printers) {
        ...
        let effectiveConnectionType = dbPrinter.connection_type || dbPrinter.printer_type;
        ...
        const printerConfig = {
          id: dbPrinter.id,
          name: dbPrinter.name,
          printer_type: effectiveConnectionType === 'usb' ? 'usb' : 'ip',
    ...
    try {
      ...
      if (this.checkOnlineStatus()) {
        ...
        return {
          ...
          connection_type: printerType === 'usb' ? 'usb' : 'ethernet',
          ip_address: ipAddr,
          ip: ipAddr,
          port: defaultPrinter.port || 9100,
          usb_port: usbPort,
          usb_device_path: usbPort,
          id: defaultPrinter.id
        }
      }

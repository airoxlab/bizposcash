...
async createOrder(orderData) {
  ...
  if (orderData.isModifying && orderData.existingOrderId) {
    console.log('🔄 [CacheManager] Modifying existing order:', orderData.existingOrderId)

    // Find the existing order in cache
    const existingOrderIndex = this.cache.orders.findIndex(o => o.id === orderData.existingOrderId)
    ...
    if (existingOrderIndex >= 0) {
      ...
      console.warn('⚠️ [CacheManager] Existing order not found in cache, creating new order')
    }
  }
  ...
  const order = {
    ...
    order_source: orderData.order_source || 'POS', // Track order source (POS, Website, Mobile App, etc.)
    original_order_source: orderData.original_order_source || orderData.order_source || 'POS', // Original source (never changes)
    is_approved: orderData.is_approved !== undefined ? orderData.is_approved : true, // Approval status (default true for POS)
    approved_at: orderData.approved_at || null, // When was it approved
    approved_by_cashier_id: orderData.approved_by_cashier_id || null, // Who approved it
    approval_notes: orderData.approval_notes || null, // Approval/rejection notes
    subtotal: orderData.subtotal,
    discount_amount: orderData.discount_amount || 0,
    discount_percentage: orderData.discount_percentage || 0,
    ...
    delivery_address: orderData.delivery_address || null, // Store delivery address
    ...
    loyalty_points_redeemed: orderData.loyalty_points_redeemed || 0, // Loyalty points used
    loyalty_discount_amount: orderData.loyalty_discount_amount || 0, // Loyalty discount applied
    order_date: (() => {
  ...
  if (this.isOnline) {
    console.log(`🌐 [createOrder] Device is online - attempting to sync order ${orderNumber} to database`)
    const syncResult = await this.syncOrder(order)

    if (syncResult && syncResult.success) {
      console.log(`✅ [createOrder] Order ${orderNumber} successfully synced to database`)
    } else {
      console.error(`❌ [createOrder] Failed to sync order ${orderNumber}:`, syncResult?.error || 'Unknown error')
      console.warn(`⚠️ [createOrder] Order saved to cache and will sync later`)
      // Mark order as needing sync
      order._isSynced = false
      order._syncError = syncResult?.error || 'Unknown sync error'
      this.saveCacheToStorage()
    }
  } else {
    console.log(`📴 [createOrder] Device is offline - order ${orderNumber} saved to cache`)
  }
 ...
 async syncOrder(order) {
  try {
    ...
    if (order.id && isValidUUID(order.id)) {
      ...
      if (existingOrder && !checkError) {
        ...
        const { data, error: orderError } = await supabase
          ...
          approved_by_cashier_id: order.approved_by_cashier_id || null, // Who approved it
        ...
        // 🆕 CRITICAL FIX: Manually deduct inventory for offline orders inserted as "Completed"
        // The database trigger only fires on UPDATE, not INSERT, so we need to handle this case
        if (order.order_status === 'Completed') {
          console.log(`📦 [Sync] Order was inserted as Completed - calling database deduct_order_inventory()`)
          try {
            const { data: deductionResult, error: deductionError } = await supabase.rpc('deduct_order_inventory', {
            ...
            if (deductionResult && deductionResult.success) {
              console.log(`✅ [Sync] Successfully deducted inventory via database function`)
              console.log(`   📊 Items deducted: ${deductionResult.deductions_made}`)
              console.log(`   💰 Total COGS: Rs. ${deductionResult.total_cogs}`)

              if (deductionResult.warnings && deductionResult.warnings.length > 0) {
                console.warn(`⚠️ [Sync] Warnings:`, deductionResult.warnings)
              }
            } else {
              const errorMsg = deductionResult?.error || 'Unknown deduction error'
            ...
            // Don't fail the entire sync - inventory can be adjusted manually later
          }
        }
      }
    }
    ...
    // Sync customer account ledger entry if payment method was "Account"
    if (order.payment_method === 'Account' && customerId) {
      try {
        ...
        if (existingLedgerEntry) {
          ...
          if (existingLedgerEntry.amount !== order.total_amount) {
            console.log(`💳 [Sync] Order was modified - updating ledger entry from Rs ${existingLedgerEntry.amount} to Rs ${order.total_amount}`)
            ...
            if (deleteError) {
              ...
              // Import customerLedgerManager dynamically to avoid circular dependencies
          ...
          console.log(`💳 [Sync] Creating customer ledger entry for Account payment: Rs ${order.total_amount}`)
          ...
          const { error: ledgerError } = await supabase
            ...
            .insert({
              ...
              notes: order._isOffline ? `Synced from offline - ${order.items?.length || 0} items` : `${order.items?.length || 0} items`,
              created_by: this.userId
            })
          ...
          if (ledgerError) {
            ...
            console.log(`✅ [Sync] Created customer ledger entry: Rs ${order.total_amount} (Balance: ${currentBalance} -> ${newBalance})`)

            // Show notification if this is being called from payment page (online order)
            if (!order._wasOffline && typeof window !== 'undefined') {
              const { notify } = await import('../components/ui/NotificationSystem')
              notify.success(`Added Rs ${order.total_amount.toFixed(2)} to customer account. Balance: Rs ${newBalance.toFixed(2)}`, { duration: 5000 })
            }
          }
        }
      } catch (ledgerError) {
    ...
    // Update order with real UUID from database
  ...
  try {
    ...
    for (const entry of allHistoryEntries) {
      try {
        ...
        const order = this.cache.orders.find(o => o.id === entry.orderId || o.order_number === entry.order_number)

        if (!order) {
          console.error(`⚠️ [Sync] Order not found in cache for history entry: ${entry.orderId}`)
          throw new Error(`Order not found: ${entry.orderId}`)
        }
...
async refreshTables() {
  ...
  async updateOrderStatus(orderId, newStatus, additionalData = {}) {
    ...
    // 🆕 CRITICAL FIX: Update local cache OPTIMISTICALLY (before trying to sync)
    const orderIndex = this.cache.orders.findIndex(o => o.id === orderId)
    if (orderIndex !== -1) {
      this.cache.orders[orderIndex] = {
        ...this.cache.orders[orderIndex],
        ...updateData,
        _isSynced: false // Mark as unsynced until we confirm
      }
      // Save immediately so UI updates
      this.saveCacheToStorage()

      // 🆕 CRITICAL FIX: Dispatch event to notify UI components that orders changed
      if (typeof window !== 'undefined') {
  ...
  queueStatusUpdateForSync(orderId, updateData) {
    // 🆕 CRITICAL FIX: Store order_number as fallback for lookup after sync
    const order = this.cache.orders.find(o => o.id === orderId)
    const orderNumber = order?.order_number || null
    ...
    const pendingUpdate = {
      orderId,
      orderNumber, // 🆕 Store order_number for fallback lookup
      updateData,
  ...
  // 🆕 CRITICAL FIX: Check and update current online status
  checkOnlineStatus() {
    if (typeof window !== 'undefined') {
      const currentOnlineStatus = navigator.onLine

      // Update cached state if it changed
      if (this.isOnline !== currentOnlineStatus) {
        console.log(`📡 [CacheManager] Network status changed: ${this.isOnline ? 'ONLINE' : 'OFFLINE'} -> ${currentOnlineStatus ? 'ONLINE' : 'OFFLINE'}`)
        this.isOnline = currentOnlineStatus

        // If we just came online, trigger sync
        if (currentOnlineStatus) {
          console.log('🔄 [CacheManager] Network restored, triggering sync...')
          this.syncOfflineData()
        }
      }
  ...
  async calculateCOGS(orders) {
    ...
    try {
      for (const order of orders) {
        ...
        // The database trigger already calculates and records costs when orders are completed
        ...
        if (stockHistory && stockHistory.length > 0) {
          ...
          for (const entry of stockHistory) {
            ...
            // Get the current average cost (ideally this should be stored historically)
            const averageCost = entry.inventory_items?.average_cost
              ? parseFloat(entry.inventory_items.average_cost)
              : 0

...
async createOrder(orderData) {
  ...
  if (this.isOnline) {
    ...
    try {
      const syncResult = await this.syncOrder(orderTemplate)
      ...
      if (syncResult && syncResult.success) {
        ...
        return { order: onlineOrder, orderNumber, dailySerial }
      } else {
  ...
  try {
    ...
    if (order.id && isValidUUID(order.id)) {
      ...
      if (existingOrder && !checkError) {
        ...
        // ✅ USING SAME FUNCTION AS ONLINE ORDERS: deduct_inventory_for_order
        if (order.order_status === 'Completed') {
          console.log(`📦 [Sync] Order was inserted as Completed - deducting inventory`)

          // Get order_type_id (required for inventory deduction)
          let orderTypeId = order.order_type_id

          // If no order_type_id, lookup from order_types table using order_type code
          if (!orderTypeId && order.order_type) {
            ...
            try {
              ...
              if (!lookupError && orderTypeData?.id) {
                ...
                // Update the synced order with order_type_id for future reference
                await supabase
                  .from('orders')
          ...
          // Call the SAME RPC function as online orders
          if (orderTypeId && this.userId) {
            ...
            try {
              ...
              console.log(`📦 [Sync] Deduction function returned:`, { deductionResult, deductionError })
        ...
        if (existingLedgerEntry) {
          ...
          const { error: ledgerError } = await supabase
            ...
            .insert({
              ...
              balance_after: newBalance,
              order_id: syncedOrder.id,
              description: `Order #${order.order_number} - ${order.order_type?.toUpperCase() || 'WALKIN'}${order.payment_method === 'Unpaid' ? ' (Unpaid)' : ''}`,
...
async refreshTables() {
  ...
  queueStatusUpdateForSync(orderId, updateData) {
    ...
    // Store order_number as fallback for lookup after sync
  ...
  enrichOrdersWithSerials(orders) {
    ...
    // Use UTC date for localStorage comparison (dailySerialManager stores UTC dates)
    const todayUTC = new Date().toISOString().split('T')[0]

    // Use LOCAL date for order_date comparison (order_date is stored in local timezone)
    const nowLocal = new Date()
    const todayLocal = `${nowLocal.getFullYear()}-${String(nowLocal.getMonth() + 1).padStart(2, '0')}-${String(nowLocal.getDate()).padStart(2, '0')}`

    // Helper: convert a date value to local date string
    ...
    const existingAssignments = existingData && existingData.date === todayUTC ? existingData.orderMap : {}
    ...
    // Filter only today's orders for serial assignment (using local date to match order_date field)
    const todaysOrders = orders.filter(order => {
      const orderDate = order.order_date || (order.created_at ? toLocalDateStr(order.created_at) : null)
      return orderDate === todayLocal
    })
    ...
    // Enrich all orders - only today's orders get serials (using local date)
    return orders.map(order => {
      ...
      if (!isToday) return { ...order, daily_serial: null }
